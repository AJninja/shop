...
<body <?php body_class(); ?>>
<?php if ( uomo_get_config('preload', true) ) {
	$preload_icon = uomo_get_config('media-preload-icon');
	$styles = array();
...
<div id="wrapper-container" class="wrapper-container">
	...
	<?php
		$header = apply_filters( 'uomo_get_header_layout', uomo_get_config('header_type') );
		get_template_part( 'headers/'.$header );
		if ( !empty($header) ) {
			uomo_display_header_builder($header);
		} else {
