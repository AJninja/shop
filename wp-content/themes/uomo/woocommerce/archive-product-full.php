<?php
get_header();
$sidebar_configs = uomo_get_woocommerce_layout_configs();
$check_elementor = '';

$display_mode = uomo_woocommerce_get_display_mode();
...
<?php 
	...
	if ( empty($e_template_id) ) {
		$e_template_id = uomo_get_config('shop_elementor_template');
	}
	if( !empty($e_template_id) ) {
		$check_elementor = 'has-elementor';
	}
?>
<?php if ( uomo_get_config('show_shop_top_categories') ) { ?>
    <?php
        wc_get_template_part('content-product_top_cat');
    ?>
<?php } else { ?>
	<div class="shop-normal">
		<?php do_action( 'uomo_woo_template_main_before' ); ?>
	</div>
<?php } ?>

<section id="main-container" class="page-shop <?php echo esc_attr($check_elementor); ?> <?php echo apply_filters('uomo_woocommerce_content_class', 'container');?>">

	<?php uomo_before_content( $sidebar_configs ); ?>

	<div class="row">
		<?php uomo_display_sidebar_left( $sidebar_configs ); ?>
					...
					<?php
						if ( is_product_taxonomy() ) {
							...
							<div class="category-description">
								<?php
								if ( $e_template_id ) {
									echo Uomo_Elementor_Extensions::render_page_content($e_template_id);
							...
							?>
							<div class="category-description">
								<?php echo Uomo_Elementor_Extensions::render_page_content($e_template_id); ?>
							</div>
							<?php
						}
					?>
		...
		<?php uomo_display_sidebar_right( $sidebar_configs ); ?>
