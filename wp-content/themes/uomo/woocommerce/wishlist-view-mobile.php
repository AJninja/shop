...
<ul class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo esc_attr($show_cb ? 'with-checkbox' : ''); ?>" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
    ...
    if( $wishlist && $wishlist->has_items() ) :
                            ...
                            <div class="additional-info-wrapper">
                                <?php if( $show_quantity || $show_stock_status ): ?>
                                    <table class="additional-info">
                                        ...
                                        <?php if( $show_stock_status ) : ?>
                                            <tr>
                                                <td class="label">
                                                    <?php esc_html_e( 'Stock:', 'uomo' ) ?>
                                                </td>
                                                <td class="value">
                                                    <?php echo trim($stock_status == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'uomo' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'uomo' ) . '</span>'); ?>
                                                </td>
                                            </tr>
                                        <?php endif ?>
                                    </table>
                                    ...
                                    <div class="move-to-another-wishlist">
                                        <?php if( 'select' == $move_to_another_wishlist_type ): ?>
                                            <select class="change-wishlist selectBox">
                                                <option value=""><?php esc_html_e( 'Move', 'uomo' ) ?></option>
                                                <?php
                                        ...
                                        <?php else: ?>
                                            <a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
                                                <?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'uomo' ) ) ?>
                                            </a>
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if( $show_remove_product ): ?>
                                    <div class="product-remove">
                                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'uomo' ) ); ?>"><i class="ti-close"></i></a>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
        ...
        <p class="wishlist-empty">
            <?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'uomo' ) ) ?>
        </p>
    <?php
    endif;
