...
if ( !function_exists( 'uomo_autocomplete_search' ) ) {
    function uomo_autocomplete_search() {

        if ( uomo_get_config('enable_autocompleate_search', true) ) {
            wp_enqueue_script( 'typeahead-bundle', get_template_directory_uri() . '/js/typeahead.bundle.min.js', array('jquery'), null, true);
        }
    }
}
add_action( 'init', 'uomo_autocomplete_search' );

add_action( 'wc_ajax_uomo_autocomplete_search', 'uomo_autocomplete_suggestions' );
function uomo_autocomplete_suggestions() {
