...
if ( !class_exists("Uomo_Woo_Brand") ) {
	class Uomo_Woo_Brand {

		public static function init() {
			// brand
			$tax = uomo_get_config( 'product_brand_attribute' );
			if ( !empty($tax) ) {
		...
		public static function add_brand() {
			...
			<div class="form-field">
				<label><?php esc_html_e( 'Thumbnail', 'uomo' ); ?></label>
		...
		public static function edit_brand( $term ) {
			...
			?>
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'uomo' ); ?></label></th>
				<td>
		...
		public static function brand_image_field( $image = '' ) {
			...
			<div class="upload_image_action">
	            <input type="button" class="button add-image" value="<?php esc_html_e( 'Add', 'uomo' ); ?>">
	            <input type="button" class="button remove-image" value="<?php esc_html_e( 'Remove', 'uomo' ); ?>">
	        </div>
			<?php
		}
		...
		public static function brand_columns( $columns ) {
			...
			foreach ($columns as $key => $value) {
				if ( $key == 'name' ) {
					$new_columns['image'] = esc_html__( 'Image', 'uomo' );
				}
		...
		public static function brand_column( $columns, $column, $id ) {
			if ( $column == 'image' ) {
				...
				$columns .= '<img src="' . esc_url( $image ) . '" alt="'.esc_attr__( 'Image', 'uomo' ).'" class="wp-post-image" />';
			}
		...
		public static function get_product_brands() {
		    global $product;
		    $brands_tax = uomo_get_config( 'product_brand_attribute' );
	...
	add_action( 'init', array('Uomo_Woo_Brand', 'init') );
}